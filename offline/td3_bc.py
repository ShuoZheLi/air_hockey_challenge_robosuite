...
class TrainConfig:
    ...
    # Wandb logging
    project: str = "TOUCHING_PUCK_td3bc"
    # project: str = "test"
    group: str = "TD3_BC-D4RL"
    name: str = "TD3_BC_TOUCHING_PUCK"
    checkpoints_path: Optional[str] = "TD3_BC_TOUCHING_PUCK"  # Save path
...
def eval_actor(
    ...
    for epi in range(n_episodes):
        ...
        while not done:
            ...
            state, reward, done, truncation, infos = env.step(action)
            # env.render()
            episode_reward += reward
        episode_rewards.append(episode_reward)
        if infos["terminated_reason"] == "success":
            success_count += 1

    actor.train()
    return np.asarray(episode_rewards), success_count/n_episodes
...
def train(config: TrainConfig):
    ...
    for t in range(int(config.max_timesteps)):
        ...
        if (t + 1) % config.eval_freq == 0:
            ...
            epi_return, success_rate = eval_actor(
                env,
                actor,
                device=config.device,
            ...
            epi_return_avg = epi_return.mean()
            # evaluations.append(normalized_eval_score)
            print("---------------------------------------")
            print(
                f"Evaluation over {config.n_episodes} episodes: "
                f"Success Rate: {success_rate:.3f}"
                f"avg episode return: {epi_return_avg:.3f}"
            )
            print("---------------------------------------")
            if config.checkpoints_path is not None:
                torch.save(
            ...
            wandb.log(
                {"Success Rate": success_rate,
                 "avg episode return": epi_return_avg,
