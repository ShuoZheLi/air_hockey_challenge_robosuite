"""
A convenience script to validate observations, such as
End Effector Orientation, Position, Translational Velocity, and Rotational Velocity
from the Robosuite environemnt.

Arguments:
    --fileNamePath (str): Path to and Name of the file that will contain the validation results
    --dataset str): Path to the dataset numpy file with x and y inputs
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fileNamePath",
        type=str,
        help="Path and Name of the .txt file that you want to contain Validation results inside, no need to create the file itself, e.g.: "
        "'path_to_store_results_dir/nameOfFile.txt'",
    ),
    parser.add_argument(
        "--dataset",
        type=str,
        help="Path to your dataset.npy file with x and y position inputs, e.g.: "
        "'path_to_data_dir/YOUR_DATASET'",
    )
    args = parser.parse_args()

    resultsFileName = args.fileNamePath
    currentDataset = args.dataset
